...
| rpm -qf [filename]      | 查询指定文件属于哪个 RPM 包 |
...
全屏方式按页阅读，空格|Ctrl+f 向下翻页，Ctrl+b 向上翻页，回车向下滚动一行，q 离开，读到末尾自动退出。
...
全屏方式按页阅读，pagedown 向下翻页，pageup 向下翻页，"/关键字" 向后搜索关键字，"?关键字" 则向前搜索关键字，q 离开。
...
根据关键字筛选匹配的行，参数 -i 忽略大小写，参数 -v 返回不匹配的行，参数 -n 显示行号，参数 -c 只返回匹配行数，参数 -b 把可执行文件当作文本文件搜索。
