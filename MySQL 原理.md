...
对于唯一二级索引，如果比较的值是 NULL，则不会使用 const，至多使用 ref。

```
SELECT * FROM single_table WHERE key2 IS NULL;
```

该访问方法的特点是表中最多只有一条记录符合条件。
...
该访问方法的特点是一个单点扫描区间，其中可能有多条满足条件的记录。
...
普通地使用索引执行查询，对应多个单点扫描区间或范围扫描区间，范围 `[-∞,+∞]` 不能用 range 方法。
...
全表扫描 InnoDB 表时，如果有 "GROUP BY 主键" 语句，那么此次执行也被认为使用 index 方法。

### all

全表扫描，对于 InnoDB，就是从头到尾遍历所有叶结点记录。

## 索引合并

使用索引来减少需要扫描的记录数量时，往往只会为单个索引形成扫描区间，而且，等值查找通常比范围查找需要扫描的记录数更少，但这并不总是成立，因为可能存在很多值相同的记录。

### Intersection 合并

存在一些特殊情况，MySQL 会为多个索引生成扫描区间。以下语句，单独使用 key1 或 key2 索引，都有可能发生大量无效回表，因为回表的记录可能不满足其它条件。

```
SELECT * FROM single_table WHERE key1 = 'a' AND key3 = 'b';
```

为此，MySQL 分别使用 key1 和 key2 索引进行查找，然后取两个索引的结果集的主键交集，这些主键的对应记录同时满足两边的条件，再进行回表。这种方案叫做 Intersection 合并，能够避免无效回表。

使用 Intersection 合并需要满足一个条件：从索引中获取的索引记录集合，必须按照主键值排列。因为对有序集取交集更简单，且对连续的主键值回表才不会造成大量随机 IO。
...
为此，MySQL 分别使用 key1 和 key2 索引进行查找，然后取两个索引的结果集的主键**并集**，这些主键的对应记录是所有满足条件的记录的集合，再进行回表。这种方案叫做 Union 合并，能够避免索引全表扫描。

使用 Union 合并同样需要满足从索引中获取的记录按主键值排列，原因与 Intersection 合并相同。

### Sort-Union 合并

Union 合并的条件可能有些苛刻，很多时候并不能根据条件从索引获得按主键排序的结果集。这时，MySQL 先把两个索引的结果集按主键排序，这样就能进行 Union 合并。相比 Union 合并，多了一步排序操作，这在很多时候仍然值得。注意，MySQL 没有实现 Sort-Intersection 合并。
...
表连接就是把两个表的记录两两配对，形成一个新表，然后根据条件筛选行和列，最后得到目标数据。结果集如果包含所有配对，那它就是两个表的笛卡尔积，如下所示。
...
* 确认第一个查询的表，称为驱动表，对其进行单表查询。假设以 t1 为驱动表，相关条件 t1.m1 > 1；

* 每从驱动表得到一条记录，就用该记录转换涉及两表的过滤条件为只与 t2 相关的条件，然后使用所有 t2 相关条件对 t2 单表查询，再把结果的记录分别与这个驱动表的记录连接，得到一部分结果。
...
已经知道，连接查询只访问一次驱动表，被驱动表的访问次数取决于驱动表的查询结果。若有三个表，就把前两个表的结果集作为新的驱动表，重复连接。有多少个驱动表，就有多少层循环，所以称为嵌套循环连接。这是最简单且效率最低的连接算法。

### 索引加快连接

每次访问被驱动表时，涉及两表的过滤条件中，那些驱动表的字段的值都已确定，所以访问被驱动表可以看作一次单表查询。基于单表查询知识，可以对被驱动表的字段建立索引，加快检索速度。
...
如果被驱动表数据量很大且每次都要进行全表扫描，会使 MySQL 反复从磁盘读取被驱动表。这时，MySQL 每次获得驱动表的记录将不再立即访问被驱动表，而是把结果暂存到 Join Buffer 连接缓冲区，存满后再访问被驱动表进行批量查询。这种策略称为基于块的嵌套循环连接，能够减少被驱动表的访问次数。

最好的情况是 Join Buffer 大到足够存放所有驱动表的结果，那就只需访问一次被驱动表。

<img src="https://images-1305875271.cos.ap-chengdu.myqcloud.com/mysql-9ffc4d9e.jpg" style="zoom: 80%;" />

通过启动参数或者系统变量 join_buffer_size 设置 Join Buffer 大小，默认 262144 字节，即 256 KB。
...
* I/O 成本：像 MyISAM、InnoDB 这类存储引擎，它们把数据持久化到磁盘，查询的时候就需要把数据加载到内存，期间耗费的时间称为 I/O 成本；
* CPU 成本：读取和检查记录是否满足查询条件，以及对结果集进行排序等操作耗费的时间称为 CPU 成本。

页是 InnoDB 内存与磁盘交互的基本单位，MySQL 规定读取一个页的成本是 1.0，读取并检查一条记录是否符合条件的成本是 0.2。这两个数字叫做成本常数，经常使用，其余成本常数遇到再说。

## 单表查询成本

假设有一个表 *single_table*，主键 *id*，二级索引 *key1*、*key3*，唯一二级索引 *key2*，联合索引 *key_part1* 、*key_part2* 、*key_part3*。现在分析执行以下查询语句的过程。
...
* *key3 > key2*：没有和常数比较，不能使用索引；
* *key_part1 LIKE '%hello%'*：LIKE 字符串以 % 开头，不能使用索引；
* *common_field = '123'*：该列没有建立索引。

综上，possible keys 只有 idx_key1 和idx_key2。

### 全表扫描的成本

对于 InnoDB 来说，全表扫描就是把聚簇索引的记录依次与给定的搜索条件进行比较，同时会把符合条件的记录放入结果集。因此 InnoDB 需要将聚簇索引的页面加载到内存，然后对记录进行检测。

查询成本 = I/O 成本 + CPU 成本，所以计算全表扫描的成本需要关注两个信息：

* 聚簇索引的页数
* 表的记录数

这两信息可从表的统计信息获取，执行 `SHOW TABLE STATUS LIKE 'single_table'`，返回包含以下字段：

* Rows：表的记录数量，对 InnoDB 表只是估计值，对 MyISAM 表是精确值；
* Data_length：表占用的字节数，对 InnoDB 表是页数*页的大小，对 MyISAM 表就是数据文件大小。

假如 single_table 包含 9693 条记录，占用 1589248 字节，通过 Data_length/16KB 反推表有 97 个页面。现在就能计算全表扫描成本：

* I/O 成本：97 x 1.0 + 1.1 = 98.1，1.1 是一个微调值，没有意义；
* CPU 成本：9693 x 0.2 + 1.0 = 1939.6，1.0 是一个微调值。

综上，single_table 全表扫描的成本是 98.1 + 1939.6 = 2037.7。这种计算方式比较粗糙，毕竟全表扫描很多聚簇索引的中间结点不会扫描。

### 各个索引的成本

**使用 uk_key2 成本**

两个可能使用的二级索引：idx_key1、uk_key2，查询优化器会先分析唯一二级索引，所以这里也先对 uk_key2 进行成本计算。uk_key2 对应搜索条件 key2 > 10 AND key2 < 1000，范围区间 (10,1000)。

对于使用二级缓存 + 回表的查询，计算成本需要关注两个数据：

* 扫描区间数量：优化器认为读取索引的一个区间与读取一个页的成本相同，即 1.0；
* 回表的记录数：回表就要读取聚簇索引，然后还要检查记录是否符合条件。

只有一个扫描区间，I/O 成本 1.0。至于回表的记录数量，MySQL 需先找到满足 key2 > 10 第一条记录，然后找到满足 key2 < 1000 最后一条记录，计算它们间隔多少页，如果 10 页以内，那就直接统计记录的数量。否则只沿着最左记录向右读 10 个页，算出平均每页包含多少个记录，最后粗略计算区间的记录数量。假如算出该区间大约有 95 条记录，读取它们 CPU 成本 95 x 0.2 + 0.01 = 19.01，0.01 是微调值。然后，区间每个记录还要回表，MySQL 认为回表一次相当于访问一个页面，所以回表 I/O 成本 95 x 1.0 = 95.0。回表的时候还要检测对应用户记录是否满足查询条件，CPU 成本 95 x 0.2 = 19.0。综上，使用 uk_key2 索引总的成本是 134.01。

统计区间的记录数量，必然会使用 Page Header 的 PAGE_N_RECS 属性，这是页的记录数量。另外，定位索引列大于某值或小于某值的记录，对 B+ 树来说非常快，成本忽略不计。

如何计算两个记录间隔多少个页？根据 B+ 树结构，两个记录间隔的页数，等于父结点中对应目录项记录间隔记录数量。如果间隔太大，两个记录的目录项记录甚至都不在一个结点，该怎么办呢？继续递归，计算目录项记录间隔多少页，反正 B+ 树最多 3、 4 层。间隔小于 10 页，直接遍历所有页 Page Header 就能得到间隔记录数量。

**使用 idx_key1 成本**

索引 idx_key1，搜索条件 key1 IN ('a', 'b', 'c')，对应 3 个单点区间：['a', 'a']、['b', 'b']、['c', 'c']。分析方式与上面相同，假设成本 168.21。

**尝试索引合并**

虽然 key1 和 key2 搜索条件使用 AND 连接，但对 idx_key1 和 uk_key2 都是范围查询，得到的二级索引记录不是按照主键值排列，所以无法使用 Intersection 索引合并。

**选择最优方案**

总的来看，使用 uk_key2 索引的成本最低，MySQL 选择 uk_key2 执行查询。

### 索引的统计数据

如果使用索引的查询有很多单点区间，如下所示，该查询会使用 idx_key1，但这个索引不是唯一二级索引，所以需要为每个区间计算有多少记录，按照前面的方式，这会带来很大损耗。MySQL 把直接使用索引来计算扫描区间有多少条记录的方式称为 index dive。

```
SELECT * FROM single_table WHERE key1 IN ('aa1', 'aa2', 'aa3', ... , 'zzz');
```

为此，MySQL 提供了一个系统变量 eq_range_index_dive_limit，默认值 200。当需要分析的区间小于该值，直接使用 index dive 方式进行分析，否则，使用索引的统计数据估算所有区间总的记录数量。

什么是索引的统计数据？MySQL 会为表的每一个索引维护一份统计数据，使用 `SHOW INDEX FROM 表名` 查看某个表的索引的统计数据。以下是该表各列的说明，除了 Packed 列，基本都能看懂。注意 Cardinality 列，其表示列有多少个不重复的值，对于 InnoDB 来说，这个值并不准确。

| 列名          | 说明                                                         |
| ------------- | ------------------------------------------------------------ |
| Table         | 索引所属表的名称                                             |
| Non_unique    | 索引列的值是否唯一，聚簇索引和唯一二级索引该值为 0，否则为 1。 |
| Key_name      | 索引的名称                                                   |
| Seq_in_index  | 索引列在索引中的位置，从 1 开始计数                          |
| Column_name   | 索引列名称                                                   |
| Collation     | 索引列的值按照什么排序方式存放，A 升序，NULL 无序            |
| Cardinality   | 索引列中不重复值的数量，对于联合索引 idx_key_part，key_part2 列 Cardinality 属性表示 (key_part1, key_part2) 组合不重复值的数量。 |
| Sub_part      | 存储字符串或者字节串的列，使用前面多少个字符或字节建立索引，NULL 完整索引 |
| Packed        | 索引列如何被压缩，NULL 未被压缩                              |
| Null          | 是否允许 NULL 值                                             |
| Index_type    | 所属索引的类型，常见的是 BTREE，即 B+ 树                     |
| Comment       | 索引列注释信息                                               |
| Index_comment | 索引注释信息                                                 |

单点扫描区间太多的时候，MySQL 怎么使用统计数据？首先执行 `SHOW TABLE STATUS` 获得 Rows 值，得到该表有多少条记录，然后用 Rows 除以 Cardinality，得到每个值平均有多少条记录，这个值也就是每个扫描区间平均记录数量。据此估算：所有单点扫描区间总的记录数 =  扫描区间数 * 区间平均记录数。

这种方式并不精确，有时甚至与实际情况相差巨大。
...
因为连接查询使用嵌套循环连接算法，驱动表只被访问一次，而被驱动表可能会被访问多次，取决于驱动表的查询结果。所以，两表连接查询的成本主要由以下两部分组成：

* 单次查询驱动表的成本；
* 多次查询被驱动表的成本。

驱动表的查询结果的记录条数称为驱动表的扇出（Fanout）。显然，驱动表的扇出越小，被驱动表的查询次数就越少，连接查询的总成本就越低。查询优化器要计算连接查询的成本，就得先计算驱动表的扇出。

有的扇出值计算很简单，比如以下查询，假设 s1 是驱动表，使用 idx_key2 索引执行查询，范围 (10, 1000) 有多少记录，扇出值就是多少。
...
有的扇出值比较棘手，比如以下查询，与上相比，多了一个 common_field > 'xyz' 条件，查询优化器又不会真地执行查询，所以分析 idx_key2 时，只能**猜**区间 (10, 1000) 有多少记录满足 common_field > 'xyz' 条件。
...
处于以下两种情况的时候，计算驱动表的扇出需要靠**猜**：

* 使用全表扫描查询驱动表，需要猜有多少记录满足搜索条件；
* 使用索引查询驱动表，需要猜范围区间内的有多少记录满足其它搜索条件。

这些猜的操作被 MySQL 称为条件过滤（Condition Filtering）。整个过程可能会使用到索引，也有可能用到索引统计数据，但肯定不是瞎猜，且比较复杂。目的是更准确地分析各个方案的成本，从而选出最优方案。

### 两表连接

两表连接查询的成本 = 单次访问驱动表的成本 + 驱动表扇出 * 单次访问被驱动表的成本。对于左/右外连接，驱动表固定，只需分别为驱动表和被驱动表选择成本最低的访问方法。但是，对于内连接，驱动表和被驱动的位置并不确定，需要考虑两个方面的问题：

* 不同的表作为驱动表最终的查询成本可能不同，需要考虑最优的表连接顺序；
* 分别为驱动表和被驱动表选择成本最低的访问方法。

显然，内连接查询的成本的计算更麻烦，现在对以下内连接查询进行成本分析。
...
首先，已知有两种连接顺序可以选择：s1 连接 s2，s2 连接 s1，查询优化器需要分别为这两种连接顺序计算成本最低的查询方法，然后对比获得最优方案。

**S1 作为驱动表**

以 s1 为驱动表，对应条件 s1.key2 > 10 AND s1.key2 < 1000，前面已经分析过，这个查询应使用 idx_key2 索引执行。接着分析被驱动表 s2，对应条件 s2.key2 > 1000 AND s2.key2 < 2000 和 s2.common_field = 常数，明显使用 idx_key2 索引成本最低。

所以，这个方案的总成本是：使用 idx_key2 访问 s1 成本 + s1 扇出 * 使用 idx_key2 访问s2 成本。

**S2 作为驱动表**

以 s2 为驱动表，对应条件 s2.key2 > 10 AND s2.key2 < 1000，应该使用 idx_key2 索引执行。被驱动表 s1，对应条件 s1.key2 > 1000 AND s1.key2 < 2000 和 s1.key1 = 常数，这时就比较有趣，因为既能使用 idx_key1 进行 ref 访问，还能使用 idx_key2 进行 range 访问，另外还有一个全表访问，怎么选择最优方案呢？前面提过此时会使用索引统计数据帮助分析，通常 ref 比 range 成本更低，假设选择 idx_key1 索引。

所以，这个方案的总成本是：使用 idx_key2 访问 s2 成本 + s2 扇出 × 使用 idx_key1 访问 s1 成本。

**最优方案与优化**

这样，就有两种方案可选，MySQL 自然会选择成本最低那个来执行查询。从上边的计算过程可以看出：连接查询成本的大头是"驱动表扇出 * 单次访问被驱动表的成本"，所以优化的重点是以下两个部分：
...
多表连接查询涉及多个表，同样，外连接需要考虑不同的表连接顺序。如果有 n 个表进行连接，那就有 *n!* 种连接顺序，要是逐个分析所有连接顺序，分析的成本将指数级上升，还不如全表扫描。

为此，MySQL 制定了许多策略来减少因计算不同连接顺序下的查询成本而带来的损耗。

* 提前结束某种顺序的成本分析

  MySQL 分析各种连接顺序的成本时，维护有一个全局变量记录目前最小的连接查询成本。如果分析某个连接顺序时，发现它的成本已经超过最小成本，则直接停止，继续分析下一种顺序。

* 系统变量 optimizer_search_depth

  如果连接表的数量小于这个变量，照常分析所有连接顺序，否则只分析这个值那么多的连接顺序。显然，该值越大，成本分析就越精确，但分析的成本也就越高。

* 根据某些规则直接跳过某些连接顺序

  使用系统变量 optimizer_prune_level 设置启发式规则（根据经验设置的规则），凡是不满足这些规则的连接顺序分析成本时会直接跳过，极大的减少需要分析的连接顺序。

## 调节成本常数

除了读取页和检测记录，还有许多成本常数，MySQL 把成本常数存到 engine_cost 和 server_cost 两个表中。

### server_cost

表 server_cost 存放 Server 层执行的操作会使用的成本常数，包含 4 个列：cost_name 常数名字，cost_value 常数的值，NULL 表示使用默认值；last_update 最近更新时间；comment 注释。

以下是 server_cost 保存的成本常数：

* disk_temptable_create_cost：默认 40.0，创建基于磁盘的临时表的成本，增大该值会让优化器尽量少地创建基于磁盘的临时表；
* disk_temptable_row_cost，默认 1.0，向磁盘临时表写入或读取一条记录的成本，增大该值会让优化器尽量少地创建基于磁盘的临时表；

* key_compare_cost：默认 0.1，比较两条记录的成本，常用于排序，增大该值会提升 filesort 成本从而让优化器更倾向使用索引而非 filesort 进行排序；
* memory_temptable_create_cost：默认 2.0，创建基于内存的临时表的成本；
* memory_temptable_row_cost：默认 0.2，向内存临时表写入或读取一条记录的成本；
* row_evaluate_cost：检测一条记录是否符合条件的成本，增大会让优化器更倾向使用索引而非全表扫描。

诸如 DISTINCT 查询、分组查询、Union 查询以及某些特殊条件下的排序查询，MySQL 都有可能在内部创建一个临时表用来辅助完成查询。比如 DISTINCT 查询就会创建一个带有 UNIQUE 索引的临时表，把要去重的记录逐个插入临时表得到结果集。数据量特别大时会创建基于磁盘的临时表，平时则倾向使用内存临时表。

MySQL 允许修改成本常数的值，以下语句修改 row_evaluate_cost 常数。
...
表 engine_cost 存放存储引擎层会使用的成本常数，与 server_cost 相比，多了两个列：engine_name 该成本常数适用的存储引擎名称，default 表示适合所有存储引擎；device_type 存储引擎使用的设备类型，主要用来区分机械硬盘和固态硬盘，0 表示不区分。

engine_cost 只有两个成本常数（5.7.21）：

* io_block_read_cost：默认 1.0，从磁盘上读取一个块的成本，InnoDB 一个页就是一个块，MyISAM 一个块是 4096 字节。增大该值会加重 I/O 成本，让优化器更倾向使用索引而非全表扫描；
* memory_block_read_cost：默认 1.0，从内存中读取一个块的成本，与上相似；

同样允许修改成本常数，还能插入新记录来添加只针对某种存储引擎的成本常数，以下添加 InooDB 成本常数。
