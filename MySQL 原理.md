...
对于索引，Root 页的 Page Header 部分会保存两个属性：PAGE_BTR_SEG_LEAF、PAGE_BTR_SEG_TOP，它们是这个索引对应的叶子节点段和非叶子节点段的头部信息，对应两个 Segment Header 结构。这样，我们就能知道这个索引属于哪个段，需要时直接访问对应的 INODE Entry 结构。
...
前边记录行格式有提到 row_id、trx_id、roll_pointer 这 3 个隐藏列，其中，row_id 是一定条件下自动生成的主键列，而 trx_id 表示最近一次修改该聚簇索引记录的事务的 ID，至于 roll_pointer，后几节说。
...
> 注意，这里只会针对聚簇索引进行记录，至于二级索引，它们会在修改聚簇索引时连带修改。每个二级索引页面 Page Header 部分都有一个 PAGE_MAX_TRX_ID 属性，表示最近一次修改该页面的事务的 ID。

![](https://images-1305875271.cos.ap-chengdu.myqcloud.com/mysql-fc9d7bca.jpg)

**隐藏列 roll_pointer**

最后一个隐藏列 roll_pointer，它是改动当前记录的对应 undo 日志的指针，占用 7 个字节。
...
针对不更新主键的更新操作，InnoDB 设计了一种 TRX_UNDO_UPD_EXIST_REC 类型 undo 日志，以下是它的结构示意图。
...
* TRX_UNDO_TRX_NO：事务提交之后产生的一个序号，越晚提交值越大；
* TRX_UNDO_DEL_MARKS：是否包含由于 Delete mark 操作产生的日志；
* TRX_UNDO_LOG_START：该组第一个日志的页内偏移量；
* TRX_UNDO_DICT_TRANS：该组日志是否由于 DDL 语句而产生；
* TRX_UNDO_TABLE_ID：如果上面属性为真，那么这个属性就是操作的表的 table id；
