...
| min_rec_mask | 1        | 标记 B+ 树每层最小非叶子结点，即每层最小目录项记录           |
| n_owned      | 4        | 记录代表的分组包含的记录数，这条记录的偏移量就是槽           |
| heap_no      | 13       | 记录在页面堆的序号                                           |
| record_type  | 3        | 记录的类型，0 普通记录，1 目录项记录，2 最小记录，3 最大记录 |
| next_record  | 16       | 下一条记录的真实数据的相对位置                               |
...
| --------------- | ---------------- | --------------------------------- |
| n_field         | 4                | 列的数量                          |
| 1byte_offs_flag | 1                | 使用 1 或 2 个字节保存 1 个偏移量 |
